...
toRegion region = \point -> case region of
    -- Daca punctul se afla in lista de puncte, atunci se afla in regiune
    FromPoints points -> S.fromPoints points point
    Rectangle x y -> S.rectangle x y point
    Circle r -> S.circle r point
    -- Daca punctul se afla in regiune, atunci nu se afla in complementul regiunii
    Complement region -> not $ toRegion region point
    -- Daca punctul se afla in una din regiuni, atunci se afla in reuniunea lor
    Union region1 region2 -> toRegion region1 point || toRegion region2 point
    -- Daca punctul se afla in ambele regiuni, atunci se afla in intersectia lor
    Intersection region1 region2 -> toRegion region1 point && toRegion region2 point
    -- Aplicam transformarea asupra punctului si verificam daca acesta se afla in regiune
    Transform transformation region -> S.applyTransformation (toTransformation transformation) (toRegion region) point
...
decomposeTransformation transformation = case transformation of
    ...
    Scaling s -> [Scaling s]
      {- Folosim concatMap pentru a aplica decomposeTransformation fiecarei sub-transformari
      si concatenam toate listele rezultate intr-o singura lista -}
...
{- Folosim foldr pentru a parcurge lista de transformari de la dreapta la stanga -}
fuseTransformations = foldr f []
    where
        {- Functia f primeste o transformare si o lista de transformari si
        decide cum se combina transformarile 
        Daca transformarea curenta este de acelasi tip cu prima transformare din lista,
        atunci se combina cele doua transformari si se adauga in lista rezultat -}
        f (Translation x1 y1) (Translation x2 y2 : ts) = Translation (x1 + x2) (y1 + y2) : ts
        f (Scaling s1) (Scaling s2 : ts) = Scaling (s1 * s2) : ts
        {- Daca transformarea curenta nu este de acelasi tip cu prima transformare din lista,
        atunci se adauga transformarea curenta in lista rezultat -}
        f t ts = t : ts
...
optimizeTransformations (Transform transformation region) = undefined
