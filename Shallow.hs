...
rectangle width height = \p -> let (x, y) = p in abs x <= w && abs y <= h
  where
    w = width / 2
    h = height / 2
...
-}
circle :: Float -> Region
circle radius = \p -> let (x, y) = p in sqrt (x * x + y * y) <= radius
...
plot width height region = intercalate "\n" rows
  where
    rows = [ [if inside (fromIntegral x, fromIntegral y) region then '*' else '.' | x <- [-w..w]] | y <- [h, h - 1..(-h)] ]
...
applyTransformation = \transformation region -> \point -> region (transformation point)
...
combineTransformations = (\transformations -> foldl (\t acc -> acc.t) id transformations)
