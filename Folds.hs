...
instance Show RegionAST where
    show = showRegion 0
      where
        ...
        showRegion indentLevel (R shape) = case shape of
            FromPoints points -> 
                line $ "FromPoints " ++ show points
            Rectangle w h -> 
                line $ "Rectangle " ++ show w ++ " " ++ show h
            Circle r -> 
                line $ "Circle " ++ show r
            Complement reg -> 
                line "~" ++ "\n" ++ showRegion (indentLevel + 1) reg
            Union reg1 reg2 -> 
                line "+" ++ "\n" ++ showRegion (indentLevel + 1) reg1 ++ "\n" ++ showRegion (indentLevel + 1) reg2
            Intersection reg1 reg2 -> 
                line "*" ++ "\n" ++ showRegion (indentLevel + 1) reg1 ++ "\n" ++ showRegion (indentLevel + 1) reg2
            Transform trans reg -> 
                line (show trans) ++ "\n" ++ showRegion (indentLevel + 1) reg
          where
            line str = replicate (2 * indentLevel) ' ' ++ str
...
instance Functor TransformationShape where
    ...
    fmap f transformation = 
        -- folosim pattern matching pentru a distinge între cazurile posibile
        case transformation of
            Translation tx ty -> Translation tx ty
            Scaling f -> Scaling f
            -- aplicăm funcția f pe fiecare element din listă folosind fmap
            -- și construim un nou Combine cu elementele modificate
            Combine transformations -> Combine $ fmap f transformations
...
instance Functor RegionShape where
    -- fmap :: (a -> b) -> RegionShape a -> RegionShape b
     fmap f regionShape = case regionShape of
        FromPoints points -> FromPoints points
        Rectangle w h -> Rectangle w h
        Circle r -> Circle r
        -- În cazul Complement, aplicăm funcția f asupra sub-regiunii
        Complement a -> Complement (f a)
        -- În cazul Union, aplicăm funcția f asupra ambelor sub-regiuni
        Union a1 a2 -> Union (f a1) (f a2)
        -- În cazul Intersection, aplicăm funcția f asupra ambelor sub-regiuni
        Intersection a1 a2 -> Intersection (f a1) (f a2)
        -- În cazul Transform, aplicăm funcția f asupra sub-regiunii
        Transform t a -> Transform t (f a)
...
-- folosim fmap pentru a aplica foldTransformationAST pe fiecare element din lista transformations
...
-- folosim fmap pentru a aplica foldRegionAST pe fiecare element din lista region
...
-- folosim foldTransformationAST pentru a aplica combiner-ul pe fiecare element din lista transformations
toTransformation = foldTransformationAST combiner
  where
    -- Funcția `combiner` specifică cum să se transforme fiecare tip de nod din AST într-o transformare efectivă.
    combiner :: TransformationCombiner Transformation
    -- folosim pattern matching pentru a distinge între cazurile posibile
    combiner transformation = 
        case transformation of
            -- pentru Translation și Scaling, apelăm funcțiile corespunzătoare din Shallow
            Translation tx ty -> S.translation tx ty
            Scaling f -> S.scaling f
            -- pentru Combine, apelăm funcția combineTransformations pentru a combina transformările
...
basicTransformationCount = foldTransformationAST combiner
  where
    ...
    combiner transformation = 
        case transformation of
            -- în cazul în care avem o translație sau scalare, numărul de transformări este 1
            Translation _ _ -> 1
            Scaling _ -> 1
            -- în cazul în care avem o combinație de transformări, numărul de transformări este suma transformărilor din fiecare listă
...
{-
    *** TODO ***
    ...
    Răspuns: Evaluarea leneșă poate ajuta dacă dorim să accesăm doar o componentă a unei 
    perechi întrucât,se amână calculul componentelor până când sunt necesare efectiv. 
    Astfel, dacă dorim să accesăm doar prima componentă a unei
    perechi, vom evalua doar acea componentă și vom ignora evaluarea celeilalte până 
    când aceasta devine necesară. În contextul funcției basicEntityCount, evaluarea
    leneșă este utilă în situația în care dorim doar să extragem numărul de regiuni
    sau numărul de transformări elementare din rezultatul funcției. Dacă dorim doar unul
    dintre aceste numere, evaluarea leneșă ne permite să evităm calculul inutil al celuilalt
    număr până când este necesar.
